import { buildDefaultCountryModule, createCountryIssue } from "@/lib/invoicing/blueprint/module-factory";

export const saintMartinComplianceModule = buildDefaultCountryModule("MF", {
  implementationType: "SPECIALIZED",
  ruleVersion: "2026-04-07",
  evidence: [
    {
      label: "EU Commission: Saint Martin outside EU VAT rules",
      url: "https://taxation-customs.ec.europa.eu/taxation/vat/vat-directive/how-does-vat-work/territorial-scope_en",
      reviewedAt: "2026-04-07",
    },
  ],
  extendValidation(issues) {
    issues.push(
      createCountryIssue(
        "invoice.countryContext",
        "MF_LOCAL_TAX_RULES",
        "Saint Martin is outside EU VAT rules; apply local invoicing requirements.",
        "WARNING",
        "MF"
      )
    );
    return issues;
  },
});
