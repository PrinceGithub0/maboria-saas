...
export const dianProvider: EInvoiceProviderAdapter = {
  key: "CO_DIAN",
  countries: ["CO"],
  documentFormat: "UBL_XML",
  supportsClearance: true,
  buildPayload(context) {
    const preparation = buildDianOnboardingPreparation(context.connection);
    const supplier = context.business || {};
    const customer = context.customer || {};
    const invoiceLines = (context.items || []).map((item, index) => buildLine(item, index));
    const warnings = [
      "Colombia DIAN payload is structured for readiness and validation and can be submitted when a DIAN endpoint is configured.",
      ...preparation.notes,
    ];

    if (!trimOrNull(supplier.legalName)) warnings.push("Supplier legal name is missing for Colombia DIAN.");
    if (!trimOrNull(supplier.taxId)) warnings.push("Supplier NIT is missing for Colombia DIAN.");
    if (!trimOrNull(supplier.addressLine1)) warnings.push("Supplier address line 1 is missing for Colombia DIAN.");
    if (!trimOrNull(customer.legalName || customer.contactName)) warnings.push("Customer legal name is missing for Colombia DIAN.");
    if (context.compliance?.buyerType === "B2B" && !trimOrNull(customer.taxId)) {
      warnings.push("Buyer NIT is missing for a Colombia B2B invoice.");
    }
    if (!invoiceLines.length) warnings.push("Colombia DIAN payload needs at least one invoice line.");
    if (!preparation.onboardingReady) {
      warnings.push(`Colombia DIAN onboarding is incomplete: ${preparation.missingArtifacts.join(", ")}.`);
    } else if (!preparation.signingReady) {
      warnings.push(`Colombia DIAN signing prerequisites are incomplete: ${preparation.missingArtifacts.join(", ")}.`);
    } else if (!preparation.transmissionReady) {
      warnings.push(`Colombia DIAN transmission prerequisites are incomplete: ${preparation.missingArtifacts.join(", ")}.`);
    }
    ...
    if (!context.transportDocument?.documentBase64) {
      warnings.push("Signed DIAN UBL XML is not attached to this invoice yet.");
    }
    if (!context.transportDocument?.uuid && !context.transportDocument?.invoiceHash) {
      warnings.push("DIAN tracking identifier or CUFE reference is not attached to this invoice yet.");
    }

    return {
      externalId: trimOrNull(context.invoiceNumber) || trimOrNull(context.invoiceId) || "dian-draft",
      format: "UBL_XML",
      payload: {
        documentProfile: "CO_DIAN",
        documentType: "DIAN",
  ...
  validatePayload(payload, context) {
    ...
    if (!trimOrNull(supplier.legalName)) errors.push("Supplier legal name is required for Colombia DIAN.");
    if (!trimOrNull(supplier.taxId)) errors.push("Supplier NIT is required for Colombia DIAN.");
    if (!trimOrNull(supplier.addressLine1)) errors.push("Supplier address line 1 is required for Colombia DIAN.");
    if (!trimOrNull(customer.legalName || customer.contactName)) errors.push("Customer legal name is required for Colombia DIAN.");
    if (!invoiceLines.length) errors.push("At least one invoice line is required for Colombia DIAN.");

    if (context.compliance?.buyerType === "B2B" && !trimOrNull(customer.taxId)) {
      warnings.push("Buyer NIT is missing for Colombia B2B invoicing.");
    }
    if (!preparation.onboardingReady) {
      warnings.push(`Colombia DIAN onboarding is incomplete: ${(preparation.missingArtifacts || []).join(", ")}.`);
    } else if (!preparation.signingReady) {
    ...
    invoiceLines.forEach((line, index) => {
      if (!trimOrNull(line.classificationCode)) {
        warnings.push(`Line ${index + 1} is missing a classification code for Colombia DIAN.`);
      }
      if (!trimOrNull(line.taxCategory)) {
        warnings.push(`Line ${index + 1} is missing a Colombia tax category.`);
      }
    });
  ...
  buildWarnings(context) {
    const preparation = buildDianOnboardingPreparation(context.connection);
    const warnings = [
      "Colombia may require DIAN submission and validation in addition to issuing a PDF invoice.",
      ...preparation.notes,
    ];
    if (!preparation.onboardingReady) {
      warnings.push(`Colombia DIAN onboarding is incomplete: ${preparation.missingArtifacts.join(", ")}.`);
    ...
    if (!trimOrNull(context.business?.taxId)) {
      warnings.push("Supplier NIT should be completed before Colombia DIAN preparation.");
    }
    if (context.compliance?.buyerType === "B2B" && !trimOrNull(context.customer?.taxId)) {
      warnings.push("Buyer NIT should be completed before Colombia B2B invoicing.");
    }
    if (!context.transportDocument?.documentBase64) {
      warnings.push("Attach the signed DIAN UBL XML before enabling live DIAN submission.");
    }
    return warnings;
  },
  async submit(payload, context) {
    return submitDianDocument({
      connection: context.connection,
      payload,
    });
  },
  async getStatus(submissionId, context) {
    return getDianSubmissionStatus({
      connection: context.connection,
      submissionId,
    });
  },
  async cancel(submissionId, context) {
    return cancelDianDocument({
      connection: context.connection,
      submissionId,
    });
  },
