...
function formatPlanPrice(currency: string, price: number | null) {
  ...
  const formatted = roundPricingDisplayAmount(price).toLocaleString(undefined, {
...
export default async function BillingLockedPage() {
  ...
  const price = await getPlanPriceForIntervalLive(plan, currency, interval);
  ...
  const planDescription =
    ...
    t(
      ...
      "Fahre mit deinem ausgewählten Plan fort, um alle Premium-Funktionen freizuschalten.",
        ...
        <div className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_14px_40px_-28px_rgba(15,23,42,0.28)] sm:p-8">
          {impersonation ? (
            ...
            {actorCanManageSubscription
              ...
              : t("Workspace Locked", "Espace de travail verrouille", "Workspace gesperrt", "Espacio de trabajo bloqueado", "Espaço de trabalho bloqueado")}
          </div>
          ...
          <p className="mt-3 text-sm text-slate-600 sm:text-base">
            {actorCanManageSubscription
              ...
              : `${t("This workspace is currently unavailable because its subscription is inactive. Contact", "Cet espace de travail est actuellement indisponible car son abonnement est inactif. Contactez", "Dieser Workspace ist derzeit nicht verfügbar, weil sein Abonnement inaktiv ist. Kontaktiere", "Este espacio de trabajo no est? disponible porque su suscripción esta inactiva. Contacta con", "Este espa?o de trabalho esta indisponível porque a sua subscrição esta inativa. Contacte")} ${
          ...
          {actorCanManageSubscription ? (
            <section className="mt-8 rounded-xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">{t("Selected Plan", "Plan selectionne", "Ausgewählter Plan", "Plan seleccionado", "Plano selecionado")}</p>
          ...
          <section className="mt-8 border-t border-slate-200 pt-6">
            ...
            <p className="mt-2 text-sm text-slate-600">
              {actorCanManageSubscription
                ? hasPendingRenewalRedirect
                  ? t("A Flutterwave renewal is already in progress for this workspace.", "Un renouvellement Flutterwave est déjà en cours pour cet espace de travail.", "Für diesen Workspace läuft bereits eine Flutterwave-Erneuerung.", "Ya hay una renovacion de Flutterwave en curso para este espacio de trabajo.", "Ja existe uma renovacao Flutterwave em curso para este espa?o de trabalho.")
                  ...
                  : t("Your previous payment attempt was not completed.", "Votre pr\u00e9c\u00e9dente tentative de paiement n'a pas \u00e9t\u00e9 finalis\u00e9e.", "Dein vorheriger Zahlungsversuch wurde nicht abgeschlossen.", "Tu intento de pago anterior no se completo.", "A sua tentativa de pagamento anterior não foi concluída.")
            ...
            <p className="mt-1 text-sm text-slate-500">
              {actorCanManageSubscription
                ? hasPendingRenewalRedirect
                  ...
                  : t("Retry below to activate your subscription instantly.", "Réessayez ci-dessous pour activer instantanement votre abonnement.", "Versuche es unten erneut, um dein Abonnement sofort zu aktivieren.", "Vuelve a intentarlo abajo para activar tu suscripción al instante.", "Tente novamente abaixo para ativar a sua subscrição instantaneamente.")
                : owner?.email
