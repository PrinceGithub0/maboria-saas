...
const FLAG_DEFS: Array<{
  ...
  { key: "automation_enabled", section: "Automation", label: text("Automation Engine", "Moteur d'automatisation", "Automatisierungs-Engine", "Motor de automatización", "Motor de automação"), description: text("Allow automation processing and execution.", "Autoriser le traitement et l'execution de l'automatisation.", "Verarbeitung und Ausfuhrung von Automatisierungen zulassen.", "Permitir el procesamiento y la ejecucion de automatizaciones.", "Permitir o processamento e a execucao de automatizacoes.") },
...
const DANGEROUS_CONFIRM: Record<SystemFlag, { title: CompleteLocalizedText; body: CompleteLocalizedText }> = {
  ...
  payments_enabled: {
    title: text("Change Payments Flag?", "Modifier le drapeau paiements ?", "Zahlungs-Flag andern?", "Cambiar el indicador de pagos?", "Alterar o indicador de pagamentos?"),
    body: text("Disabling payments stops checkout and subscription creation flows platform-wide.", "Desactiver les paiements arrete les flux de paiement et de creation d'abonnement sur toute la plateforme.", "Das Deaktivieren von Zahlungen stoppt Checkout- und Abo-Erstellungsablaufe plattformweit.", "Desactivar los pagos detiene los flujos de checkout y creacion de suscripciones en toda la plataforma.", "Desativar pagamentos interrompe os fluxos de checkout e criacao de subscricoes em toda a plataforma."),
  },
  impersonation_enabled: {
    title: text("Change Impersonation Flag?", "Modifier le drapeau d'usurpation ?", "Identitatswechsel-Flag andern?", "Cambiar el indicador de suplantacion?", "Alterar o indicador de representacao?"),
  ...
  automation_replay_enabled: {
    title: text("Change Automation Replay Flag?", "Modifier le drapeau de relecture d'automatisation ?", "Automatisierungs-Replay-Flag andern?", "Cambiar el indicador de repeticion de automatización?", "Alterar o indicador de repeticao de automação?"),
    ...
    try {
        ...
        throw new Error(
          String(
            (payload as { error?: string }).error ||
              t("Unable to update flag.", "Impossible de mettre a jour le drapeau.", "Flag kann nicht aktualisiert werden.", "No se puede actualizar el indicador.", "Não foi possivel atualizar o indicador.")
          )
        );
      }
      setActionStatus({
      ...
      setActionStatus({
        message: localizeAdminServerMessage(
          ...
          language,
          t("Unable to update flag.", "Impossible de mettre a jour le drapeau.", "Flag kann nicht aktualisiert werden.", "No se puede actualizar el indicador.", "NÃ£o foi possivel atualizar o indicador.")
        ),
        variant: "error",
      });
    } finally {
      ...
      if (!res.ok) {
        throw new Error(
          t(
            ...
            "Não foi possivel atualizar a cache de indicadores."
          )
        );
      }
      await mutate();
      ...
      setActionStatus({
        message: localizeAdminServerMessage(
          ...
          t(
            ...
            "Flag-Cache kann nicht aktualisiert werden.",
            "No se puede actualizar la cache de indicadores.",
            "NÃ£o foi possivel atualizar a cache de indicadores."
          )
        ),
        variant: "error",
      });
                    ...
                    <div key={item.key} className="flex items-center justify-between gap-3 rounded-xl border border-border/70 bg-muted/25 px-4 py-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-[11px] text-muted-foreground">
                          {t("Last modified:", "Derniere modification :", "Zuletzt geaendert:", "Ultima modificacion:", "Ultima alteracao:")}{" "}
