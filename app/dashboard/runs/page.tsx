...
const statusTone = (s: string) => {
  ...
  if (s === "RUNNING") return "border-amber-200 bg-amber-50 text-amber-700";
...
export default function RunsPage() {
  ...
  const renderRunCard = (run: RunRecord, key?: string | null) => {
    ...
    const info = contextForRun(run);
    return (
      <article key={key || run.id} data-run-menu className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:bg-slate-50/60">
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,0.8fr)_auto]">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <p className="text-sm font-semibold text-slate-900">{run.flow?.title || "Untitled automation"}</p>
              <p className="text-xs text-slate-600">Started by: {info.startedBy}</p>
          ...
          <div className="grid gap-1 text-xs text-slate-600 sm:grid-cols-3 lg:grid-cols-1">
            <div>
              <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Started</p>
              <p className="mt-0.5 text-sm text-slate-800">{formatDateTime(run.startedAt || run.createdAt)}</p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500">Duration</p>
              <p className="mt-0.5 text-sm text-slate-800">{formatDuration(run.startedAt || run.createdAt, run.completedAt, run.runStatus)}</p>
            </div>
          </div>
          <div className="flex items-start justify-end gap-2">
            ...
            <button type="button" onClick={() => { setSelectedRunId(run.id); setDrawerOpen(true); }} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-800 hover:bg-slate-100">View Details</button>
            {status === "FAILED" ? (
            ...
            <div className="relative">
              <button type="button" onClick={() => setMenuOpenId(menuOpenId === run.id ? null : run.id)} className="rounded-lg border border-slate-300 p-1.5 text-slate-600 hover:bg-slate-100">
    ...
    <div className="space-y-6">
      <header className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{t("Automation", "Automatisation")}</p>
        ...
        <p className="text-xs text-slate-500">
          {lastRefreshed ? `Last updated ${formatDateTime(lastRefreshed)}` : "Live data"}
        </p>
      </header>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-xl border border-emerald-100 bg-white p-4 shadow-sm">
          <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Completed Today</p>
        ...
        <article className="rounded-xl border border-rose-100 bg-white p-4 shadow-sm">
          <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Failed Today</p>
          <p className="mt-2 text-3xl font-semibold text-slate-900">{failedToday}</p>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-xs text-slate-500">7-day trend</span>
        ...
        <article className="rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
          <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Average Duration</p>
          <p className="mt-2 text-3xl font-semibold text-slate-900">{avgDuration}</p>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-xs text-slate-500">7-day trend</span>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        >
          <input
            ...
            className="h-10 rounded-lg border border-slate-300 px-3 text-sm text-slate-900 placeholder:text-slate-500 focus:border-blue-500 focus:outline-none"
          />
          <select
            value={statusFilter}
      ...
      <section className="space-y-3">
        {isInitialLoading ? (
          ...
          <div className="rounded-xl border border-slate-200 bg-white px-4 py-10 text-center">
            <p className="text-lg font-semibold text-slate-900">No automation runs yet.</p>
            <p className="mt-2 text-sm text-slate-600">When automations begin running, they will appear here.</p>
          </div>
        ) : !filteredRuns.length ? (
          <div className="rounded-xl border border-slate-200 bg-white px-4 py-10 text-center">
            <p className="text-lg font-semibold text-slate-900">No automation runs match your filters.</p>
            <button
              type="button"
          ...
          <div
            ...
            className="max-h-[68vh] overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 p-3"
          >
      ...
      {drawerOpen && selectedRun ? (
        <div className="fixed inset-0 z-50">
          ...
          />
          <aside className="absolute inset-x-0 bottom-0 top-12 overflow-y-auto rounded-t-2xl border border-slate-200 bg-white p-5 shadow-2xl md:inset-y-0 md:left-auto md:right-0 md:top-0 md:w-[560px] md:rounded-none md:rounded-l-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Run Overview</p>
            ...
            {(() => {
              ...
              return (
                <div className="mt-4 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 sm:grid-cols-2">
                  <div>
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Start Event</p>
                    <p className="mt-1 font-medium text-slate-900">{info.startedBy}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Customer</p>
                    <p className="mt-1 font-medium text-slate-900">{info.customer}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Invoice</p>
                    <p className="mt-1 font-medium text-slate-900">{info.invoice}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Payment Reference</p>
                    <p className="mt-1 font-medium text-slate-900">{info.paymentReference}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Started</p>
                    <p className="mt-1 font-medium text-slate-900">{formatDateTime(selectedRun.startedAt || selectedRun.createdAt)}</p>
                  </div>
                  <div>
            ...
            <div className="mt-6">
              <h3 className="text-sm font-semibold text-slate-900">Step Timeline</h3>
                ...
                {timeline.length ? (
                  timeline.map((item, idx) => (
                    <div key={item.id} className="rounded-xl border border-slate-200 bg-white p-3">
                      ...
                      {item.message ? <p className="mt-2 text-xs text-slate-600">{item.message}</p> : null}
                    </div>
                  ))
                ) : (
      ...
      {confirmRetryId ? (
        <div className="fixed inset-0 z-[60]">
          ...
          <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            <h3 className="text-lg font-semibold text-slate-900">Confirm Full Run Retry</h3>
