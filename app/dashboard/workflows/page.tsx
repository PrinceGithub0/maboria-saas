...
export default function WorkflowsPage() {
  ...
  const statusLabel = (status: WorkflowStatus) => {
    ...
    if (status === "PAUSED") return t("Paused", "En pause");
    if (status === "ARCHIVED") return t("Archived", "Archive");
    ...
    <div className="mx-auto w-full max-w-[1160px] space-y-6">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Button onClick={() => router.push("/dashboard/workflows/new")}>
          ...
          {t("New workflow", "Nouveau workflow")}
        </Button>
      </section>
      ...
      {!showUpgradeState && error ? (
        <Alert variant="error">
          {error instanceof Error ? error.message : t("Unable to load workflows.", "Impossible de charger les workflows.")}
        </Alert>
      ) : null}
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Card className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{t("Total", "Total")}</p>
          ...
          <Card className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{t("Active", "Actifs")}</p>
          ...
          <Card className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{t("Paused", "En pause")}</p>
          ...
          <Card className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{t("Actions", "Actions")}</p>
      ...
      {!showUpgradeState && !isLoading && !error && workflows.length === 0 ? (
        <Card className="py-14 text-center">
          ...
          <h2 className="mt-4 text-lg font-semibold text-slate-900 dark:text-slate-50">
            {t("No workflows yet", "Aucun workflow pour le moment")}
          </h2>
          ...
          <div className="mt-6">
            <Button onClick={() => router.push("/dashboard/workflows/new")}>
              <Plus className="h-4 w-4" />
              {t("Create workflow", "Creer un workflow")}
            </Button>
          </div>
        </Card>
      ) : null}
          ...
          {workflows.map((workflow) => (
            <Card key={workflow.id} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3 rounded-xl border border-border/70 bg-muted/40 p-3 text-center">
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-slate-500">{t("Triggers", "Declencheurs")}</p>
                  <p className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-50">{workflow.triggers.length}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-slate-500">{t("Actions", "Actions")}</p>
                  <p className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-50">{workflow.actions.length}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-slate-500">{t("Updated", "Mis a jour")}</p>
              ...
              <div className="flex justify-end">
                <Button variant="secondary" onClick={() => router.push(`/dashboard/workflows/${encodeURIComponent(workflow.id)}`)}>
                  {t("Edit workflow", "Modifier le workflow")}
                </Button>
              </div>
            </Card>
          ))}
