...
Full-stack automation, billing, and AI platform built with Next.js 14 (App Router), TypeScript, TailwindCSS, Prisma, PostgreSQL, NextAuth, Flutterwave, Paystack, and OpenAI.
...
- Dashboards: user overview, automations, runs, invoices, payments, assistant, settings; admin metrics/users/logs/support/analytics.
...
- `app/` – pages & API routes
  - `api/` – auth (signup/reset/2fa), automation (run/schedule), payments (flutterwave/paystack + webhooks), invoices (pdf), ai/assistant, admin (overview/users/logs/support), business, usage, notifications, transactions, webhooks ingest
  - `dashboard/` – overview, automations, runs, invoices, payments, assistant, settings
  - `admin/` – main, metrics, logs, support
  - `onboarding/` – business onboarding
- `components/` – UI library (buttons, inputs, cards, tables, tabs, modal, alerts, dropdown, sidebar/navbar, badges), charts, automation builders, assistant chat
...
- Webhook listener at `/api/webhooks/ingest?path=/path`.
- Scheduler `/api/automation/schedule`, run executor `/api/automation/run`, retry queue stub in `lib/jobs.ts`.

## Billing
- Flutterwave checkout `/api/payments/flutterwave`, Paystack init `/api/payments/paystack`, webhooks recorded to `Payment`.
