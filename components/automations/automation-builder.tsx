...
export function AutomationBuilder({ mode, automationId }: AutomationBuilderProps) {
  ...
  useEffect(() => {
    ...
    if (!safeAutomationId) {
      setStatus("Invalid automation link.");
      setIsHydratingEdit(false);
      return;
    }
    ...
    const hydrateFromAutomation = async () => {
      ...
      try {
        ...
        if (!active) return;
        setStatus("Unable to load automation. Please try again.");
      } finally {
  ...
  useEffect(() => {
    ...
    if (userPlan === "free") {
      setStatus("Upgrade required to use automation templates.");
    ...
    setStatus(null);
    setHydratedTemplateId(templateId);
  }, [mode, planReady, templateParam, userPlan, hydratedTemplateId]);
      ...
      setStatus(
        resolveFriendlyApiMessage(
          ...
          mode === "edit"
            ...
            : "Unable to save automation. Please try again.",
        ...
        <header className="space-y-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="ml-auto flex max-w-sm flex-col items-end">
              ...
              >
                {loading ? "Saving..." : mode === "edit" ? "Save Changes" : "Save Automation"}
              </button>
            </div>
          </div>
          <div>
            ...
            <p className="mt-2 text-base text-slate-600">
              {mode === "edit"
                ? "Update this automation and save your changes."
                : "Automate tasks so your business runs automatically."}
            </p>
          </div>
        </header>
        {isHydratingEdit ? (
